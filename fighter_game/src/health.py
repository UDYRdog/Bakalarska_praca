...
class EnergyBar:
    def __init__(self, orientation=True):
        ...
        self.front = pygame.image.load(os.path.join(base_dir, 'res', 'energyfront.png')).convert_alpha()
        self.back = pygame.image.load(os.path.join(base_dir, 'res', 'energyback.png')).convert_alpha()
    ...
    def loseHp(self, damage, damage_reduce):
        ...
        if self.health < self.maxHealth/2:
            if self.health < self.maxHealth*3/10:
                ...
                else: damage -= damage*20/100 
            else: damage -= damage*10/100 
        if damage < 2:
                damage = 2
